...
class Cribbage:
    ...
    def playGame(self):
        ...
        print("{} wins! The final score was ".format(
            self.players[self.checkWin() - 1].getName()) + self.scoreString())
    ...
    def deal(self):
        ...
        dealOrder = [x % len(self.players) for x in range(
    ...
    def cut(self, card=None):
        ...
        if self.starter.rank is Rank.Jack:
            ...
            if self.verbose:
                print("{} scores 2 for nobs!".format(
                    self.players[self.dealer].getName()))
    ...
    def scoreHands(self):
        ...
        for player in self.players:
            player.learnFromHandScores([getScore(self.players[0].hand, self.starter, False), getScore(
                self.players[1].hand, self.starter, False), getScore(self.crib, self.starter, False)], self.gameState())

    # Play the pegging phase of the game
    def play(self):
        if self.verbose:
            print("{} dealt this hand.".format(
                self.players[self.dealer].getName()))
        ...
        while (any(len(player.playhand) > 0 for player in self.players)) and (not (self.checkWin())):
            ...
            while (count < 31) and (goCounter < 2) and (not (self.checkWin())):
                if self.verbose:
                    print("It is {}'s turn. Score is ".format(
                        self.players[toPlay].getName()) + self.scoreString())
                ...
                if playedCard is None:
                    ...
                    if not (criticCard is None):
                        if not (criticCard.isIdentical(playedCard)):
                            ...
                            print("{} agrees with {}'s play.".format(
                                self.critic.getName(), self.players[0].getName()))
                    ...
                    if self.verbose:
                        print("\t{}: ".format(count) +
                              cardsString(self.inplay))
                    self.players[toPlay].pips += scoreCards(
                        self.inplay, self.verbose)
                    goCounter = 0
    ...
    def renderGame(window):
        ...
        text = font.render(str(len(self.players.hand)) +
                        " cards", True, (255, 255, 255))
